...
class TabNavigator extends ChangeNotifier {
  ...
  /// Removes the most recent page (represented by a `TabItem`) from the
  /// navigation stack, effectively navigating back to the previous page.
  void pop() {
    if (_navigationStack.length > 1) _navigationStack.removeLast();
    notifyListeners();
  }

  /// Closes all navigation stacks and returns to the `root page` in your custom
  /// tab navigation.
  void popToRoot() {
    _navigationStack
      ..clear()
  ...
  /// Removes a specific page (represented by a `TabItem`) from the navigation
  /// stack.
...
/// The `TabNavigatorProvider` class is an `InheritedNotifier` widget that
/// serves as a means to provide the `TabNavigator` instance to the widget
/// tree, allowing widgets within the tree to access it. This is a common
/// pattern in Flutter for sharing data or state across different parts of
/// your application without having to pass it explicitly through constructor
/// parameters.
